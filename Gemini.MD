...
**Palette:** Moss `#2E4036` (Primary), Clay `#C5B833` (Accent), Cream `#F2F0E9` (Background), Charcoal `#1A1A1A` (Text/Dark)  
...
**Typography:** Key Headings: "Inter" (tight tracking). Drama: "Playfair Display" Italic. Data: "JetBrains Mono".  
...
**Palette:** Deep Void `#0A0A14` (Primary), Plasma `#7B61FF` (Accent), Ghost `#F0EFF4` (Background), Graphite `#181B1B` (Text/Dark)  
...
- All buttons must have a **magnetic** feel: subtle `scale(1.03)` on hover with `cubic-bezier(0.25, 0.46, 0.45, 0.94)` timing.
...
- **Transparency Logic:** Transparent with light text at hero top. Transitions to `bg-[background]/60 backdrop-blur-xl` with primary-colored text and a subtle `border` when scrolled past the hero. Use `IntersectionObserver` or `ScrollTrigger`.
- Contains: Logo (brand name as text), 3–4 nav links, CTA button (accent color).

### B. HERO SECTION — "The Opening Shot"

- `100vh` height. Full-bleed background image (sourced from Unsplash matching preset's `imageMood`) with a heavy primary-to-black gradient overlay (`bg-gradient-to-t`).
- **Layout:** Content pushed to the bottom-left third using flex + padding.
- **Keyline Statement:** Large scale contrast following the preset's hero line pattern. First part in bold sans heading font. Second part in massive serif italic drama font (3–5x size difference).
...
- Full-width section with the **darkest color** as background.
- A parallaxing organic texture image (Unsplash, `imageMood` keywords) at low opacity behind the text.

- **Typography:** Two contrasting scales in the line pattern:
  - "Most [industry] focuses on: [common approach]." — neutral, smaller.
  - "We do: [differentiated approach]." — massive, drama serif italic, accent-colored keyword.
...
- **No placeholders.** Every prop, every label, every animation must be fully implemented and functional.
